...
USING_NS_CC;
int minionNum = 0;
float m_atktime = 0, e_atktime=0;
float mt_atktime = 0,et_atktime=0, deathtime = 0;
	...
	while (1) {
		...
		if (buf2[0] == 'm') {
			...
			if (buf2[2] == 'e') {
				...
				//heroObj->BeAttack(eneObj->AttackPower());
			}
		}
	}
}

void sendToServer(const char* order,int length){
	send(_sock, order, length, 0);
}
...
void GameScene2::newCloseMinion(int i) {
	...
	minionbuf.minionStamp = minionNum;
...
bool GameScene2::init()
{
	...
	if (side == 1) {
		Ene.setAnimeIdentifier(eneChoice);
		Ene.HeroInit(_enemy, Vec2(x, y));
		Ene.MySide = false;
		addChild(_enemy, 2, 200);
		addChild(Ene.BloodView, 2);
	}
...
void GameScene2::update(float delta){
	...
	Ene.getPosition() = Ene.getSprite()->getPosition();
	Ene.HealthRebound(delta);
	Ene.MagicRebound(delta);
	Ene.BloodView->setCurrentProgress(Ene.healthPower());
	Ene.BloodView->setPosition(Vec2(Ene.getPosition().x, Ene.getPosition().y + 15));
	...
	if (Hero.getPosition() == prelocation) {
...
bool GameScene2::onTouchBegan(Touch* touch, Event* event)
{
	...
	if (Ene.healthPower() > 0) {
		float disMouseTower = locationInNode.distance(Ene.getPosition());
		//log("%f", disMouseTower);
		if (Hero.InRange(Ene.getPosition()) && disMouseTower < TargetMinimum&&disMouseTower < 20) {
			MouseTar = &Ene;
		}
	}
	if (MouseTar != nullptr) {
